...
The vault needs to store secrets securely on disk. Secrets must be encrypted at rest and should not be portable to other
machines without explicit export.

## Decision

Use AES-256-GCM with a master key that is sealed using a machine-derived key. The machine key is derived via HKDF-SHA256
from:
