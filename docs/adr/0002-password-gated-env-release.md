...
External applications need to consume vault secrets as environment variables. The vault's machine-binding provides
baseline security, but exposing secrets to the environment requires an additional authentication gate and time-limited
access to reduce the window of exposure.
...
The sentinel file format is `nonce (12 bytes) || AES-256-GCM ciphertext(JSON payload)`, making it opaque without vault
access.
