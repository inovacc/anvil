...
SELECT * FROM vault_secret_versions WHERE profile_name = ? AND key = ? ORDER BY version DESC;

-- name: GetSecretVersion :one
SELECT * FROM vault_secret_versions WHERE profile_name = ? AND key = ? AND version = ? LIMIT 1;
...
SELECT * FROM vault_secret_versions ORDER BY profile_name, key, version;
